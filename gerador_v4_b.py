...
Gera as páginas HTML para os CNPJs que estão no progresso.json
mas ainda não têm pasta em site-cnpj/cnpj/.
...
def gerar_html(data):
    ...
    if "ATIVA" in sit:     bc,bt = "ba","Ativa"
    elif any(x in sit for x in ("BAIXADA","INAPTA","CANCELADA")):
                            bc,bt = "bb",d["situacao"].title()
    else:                   bc,bt = "bo",d["situacao"].title()

    socios = "".join([f'<li><strong>{s.get("nome_socio","—")}</strong>'
                      f'<span>{s.get("qualificacao_socio","Sócio")}</span></li>'
                      for s in d["qsa"]]) or "<li><span>Não disponível</span></li>"
    cnaes  = "".join([f'<li><strong>{c.get("codigo","")}</strong> — {c.get("descricao","")}</li>'
                      for c in d["cnaes_secundarios"]]) or "<li><span>—</span></li>"
    cont = ""
...
def processar(cnpj):
    ...
    try:
        ...
        with open(f"{path}/index.html","w",encoding="utf-8") as f: f.write(html)
...
def main():
    ...
    # Descobre quais CNPJs não têm página ainda
