#!/usr/bin/env python3
"""
regenerar_primeiros.py — BuscaCNPJ.work
Lê o progresso.json, pega os primeiros 101 CNPJs já processados
e regenera todas as páginas HTML do zero com o visual novo.
"""
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler("regenerar.log", encoding="utf-8"),
    ],
)
...
<style>
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  @media (prefers-color-scheme: dark) {
    ...
    h1 { color: #f0f0f0; }
    .field p { color: #c9c9c9; }
    ul.data-list li { color: #c9c9c9; }
...
<div class="container">
  ...
  <div class="breadcrumb">
    ...
    <a href="{DOMAIN}/cnpj/">cnpj</a><span>/</span>
    {cnpj_f}
  </div>
...
def gerar_index(index_links: list):
    itens = "".join([
        f'<li><a href="cnpj/{c}/">{n}</a>'
        f' <span style="color:#aaa;font-size:0.85rem">— {fmt_cnpj(c)}</span></li>'
...
<script>
  ...
  document.getElementById('cnpjInput').addEventListener('keydown', function(e) {{
    ...
    with open(f"{BASE_DIR}/index.html", "w", encoding="utf-8") as f:
        f.write(content)
    log.info("index.html regenerado.")
...
{urls}
    ...
    with open(f"{BASE_DIR}/sitemap.xml", "w", encoding="utf-8") as f:
        f.write(xml)
    log.info("sitemap.xml atualizado com %d URLs.", len(processed) + 1)
...
# ─── MAIN ─────────────────────────────────────────────────────────────────────
def main():
    # Lê o progresso.json para saber quais CNPJs existem
    if not os.path.exists(PROGRESS_FILE):
        log.error("progresso.json não encontrado! Rode o gerador principal primeiro.")
        return

    with open(PROGRESS_FILE, "r") as f:
        prog = json.load(f)

    todos_processed = prog.get("processed", [])
    index_links     = prog.get("index_links", [])

    # Pega apenas os primeiros 101
    alvo = todos_processed[:101]

    log.info("=" * 60)
    log.info("BuscaCNPJ.work — Regenerador dos primeiros 101")
    log.info("CNPJs a regenerar: %d", len(alvo))
    log.info("=" * 60)
    ...
    for i, cnpj in enumerate(alvo, 1):
        log.info("[%d/%d] Buscando dados: %s", i, len(alvo), cnpj)
        ...
        while tentativas < 3:
            ...
            break  # saiu do while — seja sucesso ou 404

        if data is None:
            log.warning("  ⚠️  Sem dados para %s — pulando.", cnpj)
            falhas.append(cnpj)
            continue
        ...
        try:
            ...
            log.info("  ✅  Regenerado: %s — %s", d["cnpj"], nome[:50])
            regenerados += 1
            backoff_time = BACKOFF_BASE

        except Exception as e:
            log.error("  ❌  Erro ao gerar página %s: %s", cnpj, e)
            falhas.append(cnpj)

        time.sleep(SLEEP_OK)

    # Regenera index e sitemap usando TODOS os processados (não só os 101)
    gerar_index(index_links)
    gerar_sitemap(todos_processed)

    log.info("=" * 60)
    log.info("✅  REGENERAÇÃO CONCLUÍDA")
    log.info("Páginas regeneradas : %d / %d", regenerados, len(alvo))
    if falhas:
        log.warning("Falhas (%d): %s", len(falhas), ", ".join(falhas))
    log.info("=" * 60)
    log.info("Agora suba a pasta site-cnpj para o GitHub e faça o deploy.")
